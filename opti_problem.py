...
# Az osztály egy általános MPC algoritmust implementál.
# A példányosításhoz szükséges az időhorizont, amelyen irányítunk, az idő, amelyen a beavatkozást konstansnak vesszük, illetve a rendszer modellje.
class Problem:
    def __init__(self, x0, time_horizont, holding_time, Model, margin):
        ...
        # Állítható paraméterek, más irányításokhoz más paraméterek lehetnek megfelelőek.
        penalty_weight = 1e8  
        self.time = 0
        ...
        self.ceiling_constraints = ubg
    
    # A modellhez zajt adhatunk, így konzervatívabb beavatkozó jel szekvenciát kapunk.
    ...
    # A létrehozott optimalizációs probléma megoldása adott solver típussal és inicializált döntési változókkal.
    def get_solution(self, solver_type, x_init):
        solver = cs.nlpsol('solver', solver_type, self.nlp)
        start = time.time()
        solution = solver(lbg=self.floor_constraints, ubg=self.ceiling_constraints, x0=x_init)
        end = time.time()
        # Az osztály eltárolja a megoldáshoz szükséges időt.
        self.time = end - start
        print(solver.stats()['return_status'])
        return solution

# Az eredeti Problem osztály leszármaztatása, ahol egy adott időpont után már nem avatkozunk be.
# Azaz egy adott időpont után a beavatkozó szekvencia egységesen nulla kényszert ír elő.
    ...
    def __init__(self, x0, time_horizont, grace_time, holding_time, model, margin):
        ...
        for i in range(self.control_time):
            if i < self.control_time - np.ceil(grace_time / holding_time):
                ...
                self.ceiling_constraints[i + (self.dim + 1) * time_horizont] = max_control_value
            else:
                self.floor_constraints[i + (self.dim + 1) * time_horizont] = 0
                self.ceiling_constraints[i + (self.dim + 1) * time_horizont] = 0

...
def open_loop(noise,noise_MPC,MPC_type,time_horizont,grace_time,x0,disc,model,x_init):
    ...
    [Y_sim,x_next]=model.response(U_calculated,x0.T,noise)
    return [Y_sim,U_calculated,X]

def closed_loop(noise,noise_MPC,MPC_type,x0,disc,model):
    ...
    for i in range(int(np.ceil(total_time_horizont_extended/delta_time))):
        ...
        [Y_sim,x_next]=model.response(U_calculated[0:delta_time],x,noise)
...
def closed_loop_with_PanSim(noise_MPC,MPC_type,pansim,model,U_init):
    ...
    for i in range(int(np.ceil(total_time_horizont_extended/delta_time))):
        if total_time_horizont_extended-i*delta_time>grace_time_extended:
            [Y,U_calculated,X]=open_loop(0,noise_MPC,MPC_type,time_horizont,grace_time,x.T,1,model,x_init)
        else:
            U_calculated=np.zeros(delta_time)
        [Y_sim,x_next]=pansim.response(U_calculated[0:delta_time],delta_time)
        U_system[i*delta_time:(i+1)*delta_time]=U_calculated[0:delta_time]
